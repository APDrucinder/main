...
from agents.resume_parser import ResumeData
...
"""
        ...
        return MatchScore(
            ...
            experience_match=str(data.get("experience_match", "unknown")),
            reasoning=str(data.get("reasoning", "No reasoning provided")),
            should_apply=score >= self.apply_threshold,
        )
    ...
    async def score_batch(
        ...
        for job in jobs[:max_jobs]:
            try:
                ...
                logger.warning("Job scoring failed", title=job.title, error=str(exc))
                continue
    ...
    def _format_experience(self, experience) -> str:
        ...
        return "\n".join(
            [f"- {exp.role} at {exp.company}: {exp.description[:100]}" for exp in experience]
        )
