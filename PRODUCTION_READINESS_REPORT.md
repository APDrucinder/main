...
The remaining publish blockers are deployment-configuration tasks outside the repository: set the deployed HTTPS URLs, run smoke checks against those URLs, and capture/verify each user's LinkedIn session on the deployment machine before disabling dry-run for that user.
...
3. Live auto-apply must start in dry-run until LinkedIn is connected on the deployment machine.
   ...
   - Required state for first launch: `AUTO_APPLY_ENABLED=true`, `AUTO_APPLY_DRY_RUN=true`; switch dry-run off only after user-specific LinkedIn session capture passes.
...
6. LinkedIn live auto-apply requires a valid user-specific session capture.
   - Current state: platform cookies are isolated per user under `cookies/users/<user_uuid>/`.
   - Required state: capture each user's LinkedIn cookies on the deployment machine with `python cookie_manager.py --user-id <user_uuid> linkedin`, or ship a proper Connect LinkedIn flow.
...
- Capture each user's LinkedIn cookies on the deployment machine before disabling dry-run for that user.

## Publish Recommendation

Safe to deploy a dry-run production smoke release after setting the deployed HTTPS URLs in the hosting provider.

Do not disable `AUTO_APPLY_DRY_RUN` until `/platform-sessions` shows a valid LinkedIn session for the authenticated user and a signed-in manual smoke check confirms LinkedIn does not open signed out for that user.
